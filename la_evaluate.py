...
submit_file = sys.argv[1]               # ./Scores/LA/Avg/100epoch_10batchsize/Bmamba5_LA_WCE_1e-06_ES144_NE12.txt
truth_dir = sys.argv[2]                 # /home/esseo/DB/LA/keys/LA
phase = sys.argv[3]                         # eval
attack_types = ['A19','bonafide']
...
def load_asv_metrics():
    ...
    un_asv_key_data = pandas.read_csv(asv_key_file, sep=' ', header=None)
    # asv_key_data = un_asv_key_data[un_asv_key_data[4] == attack_types[0]]
    asv_key_data = un_asv_key_data
    # asv_key_data = asv_key_data.reset_index(drop=True)
    asv_scr_data = pandas.read_csv(asv_scr_file, sep=' ', header=None)
    # asv_scr_data = asv_scr_data.loc[asv_key_data.index]
...
def eval_to_score_file(score_file, cm_key_file):
    ...
    if len(submission_scores) != len(cm_data):
        ...
        exit(1)
    # cm_data = cm_data[(cm_data[4] == attack_types[0]) | (cm_data[4] == attack_types[1])]
    # cm_data = cm_data.reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    if phase != 'progress' and phase != 'eval' and phase != 'hidden':
        print("phase must be either progress, eval, or hidden")
        exit(1)
