...
class FrequencyAttention(nn.Module):
    ...
    def forward(self, wavelet_tokens):
        ...
        return weighted_wavelet  # (B, 3, hidden_dim)
...
class Model(nn.Module):
    def __init__(self, d_args):
        ...
        self.wavelet_encoder = WaveletEncoder(hidden_dim=64)
        self.freq_attn = FrequencyAttention(hidden_dim = 64)
    ...
    def cross_attention(self,q, k):
        device = q.device
        k = k.to(device)
        query = q.permute(1, 0, 2).to(device)
        key = k.permute(1, 0, 2).to(device)
        value = k.permute(1, 0, 2).to(device)
        
        feature_dim = 64
        num_heads = 8
        cross_attention_LL = nn.MultiheadAttention(embed_dim=feature_dim, num_heads=num_heads).to(device)

        # Cross-Attention 수행
        attn_output, attn_weights = cross_attention_LL(query, key, value)
    ...
    def forward(self, x, Freq_aug=False):
        ...
        HL_x = HL_x.to(x.device)
        LH_x = LH_x.to(x.device)
        HH_x = HH_x.to(x.device)
        wavelet_tokens = self.wavelet_encoder(HL_x, LH_x, HH_x) 

        wavelet_tokens = self.freq_attn(wavelet_tokens)                            # (batch_size, 64)
        wavelet_tokens = self.wavelet_dropout(wavelet_tokens)
        ...
        # (#bs, #filt, #spec, #seq)
        # e: (B, C, H, W) -> (B, H, W, C)
        e = self.encoder(x)                         # (b , 64, 23, 29)
        # e: (B, C, H, W) -> (B, H, W, C)
        e = e.permute(0, 2, 3, 1).contiguous()
        # e: (B, H, W, C) -> (B, H*W, C)  (Flatten)
        B, H, W, C = e.shape
        e = e.view(B, H * W, C)  # (B, 667, 64)  # (23 * 29 = 667)

        e = e + self.cross_attention(e, wavelet_tokens)  # (B, 667, 64)
        e = e.view(e.shape[0], 23, -1, 64).permute(0,3,1,2)
